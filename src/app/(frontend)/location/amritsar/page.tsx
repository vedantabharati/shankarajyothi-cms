...
export const metadata = { title: 'Amritsar | Shankarajyothi', description: 'Amritsar — Golden Temple city, Punjab.' }
export default async function AmritsarPage() {
  const payload = await getPayload({ config })
  const locationResult = await payload.find({ collection: 'locations', where: { qrSlug: { equals: 'loc-amritsar' } }, limit: 1 })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Amritsar'}</h1>
        <p className="location-desc">The holy city of Punjab — home to the Sri Harmandir Sahib (Golden Temple) and the historic Jallianwala Bagh.</p>
      ...
      <div className="location-section"><h2 className="location-section-heading">About This Stop</h2><p style={{ fontWeight: 500, lineHeight: 1.8 }}>Amritsar, founded in 1577 by Guru Ram Das, is the spiritual and cultural centre of the Sikh faith. The yatra visits this historic city as it journeys northward through Punjab towards Kashmir. The city is also home to the ancient Durgiana Temple, modelled after the Golden Temple and dedicated to Goddess Durga.</p></div>
