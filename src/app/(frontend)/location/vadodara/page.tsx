...
export const metadata = { title: 'Vadodara | Shankarajyothi', description: 'Vadodara — Shuklathirtha and Kalika Mata Temple.' }

export default async function VadodaraPage() {
  const payload = await getPayload({ config })
  const locationResult = await payload.find({ collection: 'locations', where: { qrSlug: { equals: 'loc-vadodara' } }, limit: 1 })
  ...
  const placesOfInterest = [
    { icon: '🌊', name: 'Shuklathirtha', desc: 'A sacred tirtha site on the route to Vadodara, known for its ritual significance.' },
    { icon: '🛕', name: 'Kalika Mata Temple', desc: 'An ancient temple dedicated to Goddess Kalika in the Vadodara region.' },
  ]
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Vadodara'}</h1>
        <p className="location-desc">The cultural capital of Gujarat — visiting Shuklathirtha and the Kalika Mata Temple on the yatra route.</p>
