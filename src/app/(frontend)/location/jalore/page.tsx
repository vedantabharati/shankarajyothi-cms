...
export const metadata = { title: 'Jalore | Shankarajyothi', description: 'Jalore — Rajasthan stopover.' }
export default async function JalorePage() {
  const payload = await getPayload({ config })
  const locationResult = await payload.find({ collection: 'locations', where: { qrSlug: { equals: 'loc-jalore' } }, limit: 1 })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Jalore'}</h1>
        <p className="location-desc">A town in southern Rajasthan with a historic hill fort, serving as a stopover on the yatra route through the Thar desert region.</p>
      ...
      <div className="location-section"><h2 className="location-section-heading">About This Stop</h2><p style={{ fontWeight: 500, lineHeight: 1.8 }}>Jalore is a town in southern Rajasthan, known for its impressive hilltop fort and the Topkhana Mosque, which was originally a Sanskrit school. The yatra passes through on its way to western Rajasthan.</p></div>
