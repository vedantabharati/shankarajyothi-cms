...
export const metadata = { title: 'Jaisalmer | Shankarajyothi', description: 'Jaisalmer — the Golden City of Rajasthan.' }
export default async function JaisalmerPage() {
  const payload = await getPayload({ config })
  const locationResult = await payload.find({ collection: 'locations', where: { qrSlug: { equals: 'loc-jaisalmer' } }, limit: 1 })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Jaisalmer'}</h1>
        <p className="location-desc">The Golden City — famed for its sandstone fort rising from the Thar desert, ancient Jain temples, and intricately carved havelis.</p>
      ...
      <div className="location-section"><h2 className="location-section-heading">About This Stop</h2><p style={{ fontWeight: 500, lineHeight: 1.8 }}>Jaisalmer, founded in 1156 CE by Maharawal Jaisal, is a city in the heart of the Thar desert. Its iconic golden sandstone fort (Sonar Quila) is one of the few &quot;living forts&quot; where people still reside within the walls. The city is a stopover on the yatra&apos;s Rajasthan itinerary.</p></div>
