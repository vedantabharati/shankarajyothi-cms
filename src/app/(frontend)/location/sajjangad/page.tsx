...
export const metadata = {
  title: 'Sajjangad | Shankarajyothi',
  description: 'Sajjangad — historic fort visit on the Shaankara Jyothi Prakasha yatra.',
}

export default async function SajjangadPage() {
  const payload = await getPayload({ config })

  const locationResult = await payload.find({
    collection: 'locations',
    where: { qrSlug: { equals: 'loc-sajjangad' } },
    limit: 1,
  })
  ...
  const placesOfInterest = [
    {
      icon: '🏰',
      name: 'Sajjangad Fort',
      desc: 'A hilltop fort near Satara, famously associated with the 17th-century saint-philosopher Samarth Ramdas, who spent his final years here. The fort houses his samadhi and the Shri Ram temple.',
    },
  ]
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Sajjangad'}</h1>
        <p className="location-desc">
          A sacred hilltop fort in Maharashtra associated with the saint-philosopher Samarth Ramdas — a significant halt on the route from Kolhapur to Pandharpur.
        </p>
