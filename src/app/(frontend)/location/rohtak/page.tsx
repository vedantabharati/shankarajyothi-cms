...
export const metadata = { title: 'Rohtak | Shankarajyothi', description: 'Rohtak — Haryana stopover.' }
export default async function RohtakPage() {
  const payload = await getPayload({ config })
  const locationResult = await payload.find({ collection: 'locations', where: { qrSlug: { equals: 'loc-rohtak' } }, limit: 1 })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Rohtak'}</h1>
        <p className="location-desc">A city in Haryana serving as a stopover on the yatra&apos;s northward journey towards Kurukshetra and Punjab.</p>
      ...
      <div className="location-section"><h2 className="location-section-heading">About This Stop</h2><p style={{ fontWeight: 500, lineHeight: 1.8 }}>Rohtak is an ancient city in Haryana with references in the Mahabharata (as Rohitaka). It serves as a transit point on the yatra&apos;s route towards the sacred Kurukshetra and Punjab.</p></div>
