...
export const metadata = { title: 'Udaipur | Shankarajyothi', description: 'Udaipur — City of Lakes, Rajasthan.' }

export default async function UdaipurPage() {
  const payload = await getPayload({ config })
  const locationResult = await payload.find({ collection: 'locations', where: { qrSlug: { equals: 'loc-udaipur' } }, limit: 1 })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Udaipur'}</h1>
        <p className="location-desc">The City of Lakes — Rajasthan&apos;s jewel, known for its palaces, temples, and the shimmering Pichola and Fateh Sagar lakes.</p>
      ...
      <div className="location-section">
        ...
        <p style={{ fontWeight: 500, lineHeight: 1.8 }}>Udaipur, founded in 1559 by Maharana Udai Singh II, is nestled amidst the Aravalli hills in southern Rajasthan. The yatra passes through this culturally rich city on its northward journey through Rajasthan.</p>
      </div>
