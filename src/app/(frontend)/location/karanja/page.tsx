...
export const metadata = {
  title: 'Karanja | Shankarajyothi',
  description: 'Karanja — Vidarbha region visit on the Shaankara Jyothi Prakasha yatra.',
}

export default async function KaranjaPage() {
  const payload = await getPayload({ config })

  const locationResult = await payload.find({
    collection: 'locations',
    where: { qrSlug: { equals: 'loc-karanja' } },
    limit: 1,
  })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Karanja'}</h1>
        <p className="location-desc">
          A stop in the Vidarbha region of Maharashtra, connecting the Ramtek area to Chhatrapati Sambhajinagar on the yatra route.
        </p>
      ...
      <div className="location-section">
        ...
        <p style={{ fontWeight: 500, lineHeight: 1.8 }}>
          Karanja (Karanja Lad) is a town in the Washim district of Maharashtra&apos;s Vidarbha region. It is an ancient settlement with a rich cultural heritage and serves as a connector on the yatra route between the Nagpur/Ramtek area and the Marathwada region. The town has historical significance as an important centre of the Mahanubhava sect and other philosophical traditions.
        </p>
      </div>
