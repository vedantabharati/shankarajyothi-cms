...
export const metadata = {
  title: 'Shimoga | Shankarajyothi',
  description: 'Shimoga — waypoint en route to coastal Karnataka on the Shaankara Jyothi Prakasha yatra.',
}

export default async function ShimogaPage() {
  const payload = await getPayload({ config })

  const locationResult = await payload.find({
    collection: 'locations',
    where: { qrSlug: { equals: 'loc-shimoga' } },
    limit: 1,
  })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Shimoga'}</h1>
        <p className="location-desc">
          A transit waypoint in the Malnad region of Karnataka, en route from Chikkamagaluru to the coastal town of Kumta.
        </p>
      ...
      <div className="location-section">
        ...
        <p style={{ fontWeight: 500, lineHeight: 1.8 }}>
          Shimoga (Shivamogga) is a city in the Malnad region of Karnataka, surrounded by the lush Western Ghats. It serves as a transit point on the yatra route, connecting the Chikkamagaluru region to the coastal areas of Karnataka. The city is known for its proximity to the Jog Falls and the forests of the Sharavathi valley.
        </p>
      </div>
