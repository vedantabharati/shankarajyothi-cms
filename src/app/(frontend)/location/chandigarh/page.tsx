...
export const metadata = { title: 'Chandigarh | Shankarajyothi', description: 'Chandigarh — gateway to Punjab and Himachal.' }
export default async function ChandigarhPage() {
  const payload = await getPayload({ config })
  const locationResult = await payload.find({ collection: 'locations', where: { qrSlug: { equals: 'loc-chandigarh' } }, limit: 1 })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Chandigarh'}</h1>
        <p className="location-desc">The planned city at the foothills of the Shivaliks — Union Territory and gateway to Punjab and Himachal Pradesh.</p>
      ...
      <div className="location-section"><h2 className="location-section-heading">About This Stop</h2><p style={{ fontWeight: 500, lineHeight: 1.8 }}>Chandigarh, designed by the architect Le Corbusier, is a Union Territory serving as the capital of both Punjab and Haryana. The yatra stops here on its northward journey towards Punjab and Kashmir.</p></div>
