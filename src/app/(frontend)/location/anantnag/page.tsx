...
export const metadata = { title: 'Anantnag | Shankarajyothi', description: 'Anantnag — ancient Shiva shrine region, Jammu & Kashmir.' }
export default async function AnantnagPage() {
  const payload = await getPayload({ config })
  const locationResult = await payload.find({ collection: 'locations', where: { qrSlug: { equals: 'loc-anantnag' } }, limit: 1 })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Anantnag'}</h1>
        <p className="location-desc">An ancient Shiva shrine region in the Kashmir valley — named after the sacred Ananta Naga spring, one of Kashmir&apos;s most revered tirtha sites.</p>
      ...
      <div className="location-section"><h2 className="location-section-heading">About This Stop</h2><p style={{ fontWeight: 500, lineHeight: 1.8 }}>Anantnag (Islamabad) is one of the oldest towns in the Kashmir valley. Its name derives from the Ananta Naga spring — a sacred Shiva tirtha. The district is home to the famous Martand Sun Temple ruins and lies along the route to the Amarnath cave shrine.</p></div>
