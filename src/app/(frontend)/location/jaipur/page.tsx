...
export const metadata = { title: 'Jaipur | Shankarajyothi', description: 'Jaipur — Pink City, capital of Rajasthan.' }
export default async function JaipurPage() {
  const payload = await getPayload({ config })
  const locationResult = await payload.find({ collection: 'locations', where: { qrSlug: { equals: 'loc-jaipur' } }, limit: 1 })
  ...
  return (
    <>
      <section className="location-hero">
        ...
        <h1>{location?.name ?? 'Jaipur'}</h1>
        <p className="location-desc">The Pink City — Rajasthan&apos;s capital, a UNESCO World Heritage city known for its planned architecture, Amber Fort, and Hawa Mahal.</p>
      ...
      <div className="location-section"><h2 className="location-section-heading">About This Stop</h2><p style={{ fontWeight: 500, lineHeight: 1.8 }}>Jaipur, founded in 1727 by Maharaja Sawai Jai Singh II, is one of India&apos;s first planned cities. A UNESCO World Heritage site, it is known for the Amber Fort, Nahargarh Fort, Hawa Mahal, and the Jantar Mantar astronomical observatory.</p></div>
