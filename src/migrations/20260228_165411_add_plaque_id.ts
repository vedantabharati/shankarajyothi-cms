...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_expeditions_status" AS ENUM('planned', 'ongoing', 'completed');
  ...
  CREATE TABLE "locations" (
  	...
  	"plaque_installed" boolean DEFAULT false,
  	"qr_slug" varchar,
  	"plaque_id" numeric,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  ...
  CREATE TABLE "expeditions_itinerary" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  ...
  CREATE TABLE "expeditions" (
  	...
  	"status" "enum_expeditions_status" DEFAULT 'planned',
  	"summary" varchar,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  ALTER TABLE "expeditions_itinerary" ADD CONSTRAINT "expeditions_itinerary_location_id_locations_id_fk" FOREIGN KEY ("location_id") REFERENCES "public"."locations"("id") ON DELETE set null ON UPDATE no action;
  ALTER TABLE "expeditions_itinerary" ADD CONSTRAINT "expeditions_itinerary_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."expeditions"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE UNIQUE INDEX "locations_qr_slug_idx" ON "locations" USING btree ("qr_slug");
  CREATE UNIQUE INDEX "locations_plaque_id_idx" ON "locations" USING btree ("plaque_id");
  ...
  CREATE INDEX "expeditions_itinerary_order_idx" ON "expeditions_itinerary" USING btree ("_order");
  CREATE INDEX "expeditions_itinerary_parent_id_idx" ON "expeditions_itinerary" USING btree ("_parent_id");
  CREATE INDEX "expeditions_itinerary_location_idx" ON "expeditions_itinerary" USING btree ("location_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "expeditions_itinerary" CASCADE;
